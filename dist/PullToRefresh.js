 ...
 */
function PullToRefresh(el, options, IScroll) {
    var _this = this;
    ...
    var _options = _this._options = _this._extend({ // 默认选项
        ...
        scrollUpCallback: function scrollUpCallback(position) {// 滚动条向上滚动回调position：为当前移动位置

        },
        scrollDownCallback: function scrollDownCallback(position) {// 滚动条向下滚动回调position：为当前移动位置
...
PullToRefresh.prototype = {
     ...
     */
    _extend: function _extend(def, now) {
        if (def || now) {
            for (var item in now) {
                def[item] = now[item];
            }
     ...
     */
    _formatRegText: function _formatRegText(regText) {
        if (!regText) {
            return regText;
        }
     ...
     * @return {Dom} 节点
     */
    _addClassName: function _addClassName(el, className) {
        var _this = this;
        if (!el || !className) {
            return el;
        }
     ...
     * @return {Dom} 节点
     */
    _removeClassName: function _removeClassName(el, className) {
        var _this = this;
        if (!el || !className) {
            return el;
        }
     ...
     */
    _formatStyle: function _formatStyle(text) {
        // 格式化CSS3属性名
        var _this = this,
        ...
        if (!_this._memory[text]) {
            ...
            if (!ret && typeof elm.style[text] !== 'undefined') {
                // IE9有可能有些属性不加前缀也是可以，但是没有效果
                ret = '';
            }
     ...
     * 初始化iScroll插件
     */
    _iScrollInit: function _iScrollInit() {
        var _this = this,
            _el = this.el,
    ...
    */
    _getEl: function _getEl(el, parent) {
        var _el = '';
     ...
     */
    _createNode: function _createNode(html) {
     ...
     */
    _setNodeHeight: function _setNodeHeight(el, _height, _time) {
        var _this = this,
     ...
     *初始化上拉加载
    */
    pullUpInit: function pullUpInit() {
        var _this = this,
            _options = _this._options,
     ...
     *重置上拉
    */
    resetPullUp: function resetPullUp() {
        var _this = this,
            _options = _this._options;
        if (!_options.isPullUp) {
            return;
     ...
     * 清除上拉加载
     */
    clearPullUp: function clearPullUp() {
        var _this = this;
    ...
    *
    */
    pullUpFinish: function pullUpFinish() {
        var _this = this,
            _options = _this._options,
            _myIScroll = _this.myIScroll;
        if (!_options.isPullUp) {
     ...
     */
    startUpLoad: function startUpLoad(isPullUp) {
        var _this = this;
     ...
     */
    _startUpLoad: function _startUpLoad(isPullUp) {
        var _this = this,
            _options = _this._options; // ,
        // _myIScroll = _this.myIScroll;
    ...
    *@returns {Dom} 节点
    */
    _createUpNode: function _createUpNode() {
        var _this = this,
        ...
        var _domUp = _this._createNode('<div class="kids_pullUp">' + '<div class="kid_pullUp-msg-wrap">' + '<div class="kid_pullUp-msg">' + '<span class="kid_icon">&#xe640;</span>' + '<span class="kid_text">' + '上拉显示更多' + '</span>' + '</div>' + '</div>' + '</div>');
     ...
     */
    _slideDown: function _slideDown(y) {
        var _this = this,
            _options = _this._options,
    ...
    *@returns {Dom} 节点
    */
    _createDownNode: function _createDownNode() {
        var _this = this,
            _childNodes = _this.el.childNodes,
            _res,
            _domContent;
     ...
     *暂停下拉完成
    */
    _stopFinish: function _stopFinish() {
        var _this = this;
     ...
     * 初始化下拉刷新
     */
    pullDownInit: function pullDownInit() {
        var _this = this,
            _options = _this._options,
            _myIScroll = _this.myIScroll;
     ...
     * 下拉自动完成
     */
    pullDownfinishAuto: function pullDownfinishAuto() {
        var _this = this,
            _options = _this._options,
            _time = 300,
            // 动画时间
     ...
     */
    pullDownfinish: function pullDownfinish(_time) {
        // 下拉完成
        var _this = this;
    ...
    _bind: function _bind() {
        ...
        _myIScroll.on('scrollStart', function () {
            // 开始滚动页面
            _this.myIScroll.refresh();
        ...
        _myIScroll.on('scroll', function () {
            ...
            if (this.maxScrollY - this.y > 1 + Math.min(_distance, 0)) {
                // 上拉
                if (this._execEvent) {
        ...
        _myIScroll.on('scrollEnd', function () {
            // 页面滚动结束
            if (!_this._downLock) {
        ...
        _this.el.addEventListener('touchend', function () {
            // 手指离开屏
            _setTouchEnd();
        });

        _this.el.addEventListener('mouseup', function () {
            // 鼠标抬起
            _setTouchEnd();
        });
